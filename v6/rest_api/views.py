...
from .models import Word, Id, Usuario
from . import funcs
from .componentes import login, telas

@api_view(["POST"])
def chatbot_telegram(request):
    id = request.data['message']['chat']['id']
    primeira_requisicao = request.data['message']['text']
    login_user = login.Login(id, primeira_requisicao)
    if login_user == "logado":
        dados_user = Usuario.objects.filter(telegram_id=id)
        telas.Tela1()
