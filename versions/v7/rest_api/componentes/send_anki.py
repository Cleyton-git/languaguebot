...
def create_apkg(model, deck, id):
    frases = FraseUsuario.objects.all() # ARRUMAR ISSO AQUI DPS, NÃO PODE PUXAR TODAS AS FRASES
    #frases = FraseUsuario.objects.filter(usuario=id).all() < acredito que funcione
    for c in frases:
    ...
    r = requests.post("https://api.telegram.org/bot8249452727:AAExS5DziVnWEUy2kXO-pwFZ5nmhiCt2aBs/sendDocument",
                  files={"document": ("deck.apkg", apkg_file)}, data={"chat_id": id})
    return r.json()['ok']

def send_anki(user, c):
